...
const styles = `
  ...
  .animate-explode { animation: explode 1.2s cubic-bezier(0.25, 1, 0.5, 1) forwards; }
  .animate-modal { animation: modal-pop 0.3s cubic-bezier(0.16, 1, 0.3, 1) forwards; }
  .animate-scale-in { animation: scale-in-elastic 0.8s cubic-bezier(0.175, 0.885, 0.32, 1.275) forwards; }
  ...
  useEffect(() => {
    ...
    for (let i = 0; i < particleCount; i++) {
      ...
      particles.push({
        ...
        color: ["#ff0000", "#ff69b4", "#ff1493", "#e11d48", "#fb7185"][
  ...
  >
    ...
    <div className="bg-white p-2 pb-8 shadow-md rounded-sm border border-gray-100 relative h-full">
      ...
      <img
        src={src}
        alt="Memory"
        className="w-full h-40 object-cover rounded-sm filter brightness-[1.02] contrast-[1.05]"
      />
      <p className="absolute bottom-2 left-0 w-full text-center font-handwriting text-gray-500 text-xs font-medium tracking-wide px-1 truncate">
        {caption}
      </p>
    </div>
  </div>
...
const ImageModal = ({ src, onClose }) => {
  ...
  return (
    ...
    >
      ...
      <div
        className="relative animate-modal"
...
export default function App() {
  ...
  const photos = [
    {
      src: "https://images.unsplash.com/photo-1516589178581-6cd7833ae3b2?w=500&q=80",
      caption: "วันแรกที่เจอกัน",
      rotate: "-rotate-3",
    },
    {
      src: "https://images.unsplash.com/photo-1518568814500-bf0f8d125f46?w=500&q=80",
      caption: "ทริปทะเลแสนหวาน",
      rotate: "rotate-2",
    },
    {
      src: "https://images.unsplash.com/photo-1522673607200-1645062cd958?w=500&q=80",
      caption: "รอยยิ้มของคุณ",
      rotate: "-rotate-1",
    },
    {
      src: "https://images.unsplash.com/photo-1474552226712-ac0f0961a954?w=500&q=80",
      caption: "เดทแรกของเรา",
      rotate: "rotate-3",
    },
    {
      src: "https://images.unsplash.com/photo-1529333166437-7750a6dd5a70?w=500&q=80",
      caption: "เดินเล่นในสวน",
      rotate: "-rotate-2",
    },
    {
      src: "https://images.unsplash.com/photo-1518199266791-5375a83190b7?w=500&q=80",
      caption: "ของขวัญชิ้นนั้น",
      rotate: "rotate-1",
    },
    {
      src: "https://images.unsplash.com/photo-1523438885200-e635ba2c371e?w=500&q=80",
      caption: "จับมือกันไว้",
      rotate: "-rotate-3",
    },
    {
      src: "https://images.unsplash.com/photo-1621112904887-419379ce6824?w=500&q=80",
      caption: "วันเกิดปีที่แล้ว",
      rotate: "rotate-2",
    },
    {
      src: "https://images.unsplash.com/photo-1545389336-cf090694435e?w=500&q=80",
      caption: "หัวเราะด้วยกัน",
      rotate: "-rotate-1",
    },
  ];
  ...
  return (
    <div className="min-h-screen bg-gradient-to-br from-[#ffebef] via-[#fff0f5] to-[#ffe4e6] overflow-x-hidden font-sans selection:bg-rose-200">
      ...
      {(stage === "closed" || stage === "opening") && (
        <div
          className={`fixed inset-0 z-50 flex flex-col items-center justify-center transition-opacity duration-1000 ${stage === "opened" ? "opacity-0 pointer-events-none" : "opacity-100"}`}
        >
          <div className="relative cursor-pointer group" onClick={handleOpen}>
            ...
            <div
              className={`relative w-72 h-48 bg-rose-500 rounded-lg shadow-2xl flex items-center justify-center z-10 transition-transform duration-700 ${stage === "opening" ? "translate-y-32 scale-110 opacity-0" : "hover:scale-105"}`}
            >
              <div
                className={`absolute top-0 left-0 w-0 h-0 border-l-[144px] border-r-[144px] border-t-[100px] border-l-transparent border-r-transparent border-t-rose-400 rounded-t-lg z-30 envelope-flap ${stage === "opening" ? "origin-top rotate-x-180 -z-10" : ""}`}
              ></div>
              <div
                className={`absolute bottom-0 w-64 h-40 bg-white rounded-md shadow-md z-20 transition-all duration-700 ease-out flex items-center justify-center ${stage === "opening" ? "-translate-y-48 opacity-100" : "translate-y-0 opacity-0"}`}
              >
                <div className="text-center p-4">
                  <Heart
                    className="w-10 h-10 text-rose-500 mx-auto animate-bounce"
                    fill="currentColor"
                  />
        ...
        <div className="fixed inset-0 z-[120] flex items-center justify-center pointer-events-none">
          <div className="animate-scale-in">
            <div className="animate-heartbeat drop-shadow-2xl">
              <Heart
                fill="#e11d48"
                className="text-rose-600 w-64 h-64 md:w-96 md:h-96 filter drop-shadow-lg"
              />
      ...
      <div
        className={`transition-all duration-1000 ease-in-out ${stage === "opened" ? "opacity-100 translate-y-0 blur-0" : "opacity-0 translate-y-20 blur-md pointer-events-none"}`}
      >
        <div className="container mx-auto px-4 py-12 max-w-5xl relative z-10">
          ...
          <div className="text-center pb-20 relative">
            {clicks.map((c) => (
              <div
                key={c.id}
                className="fixed pointer-events-none z-50 animate-explode"
                style={{ left: c.x, top: c.y, "--x": "0px", "--y": "-100px" }}
              >
            ...
            <div className="glass-panel inline-block p-12 rounded-[3rem] shadow-2xl relative overflow-visible">
              <div
                className="absolute -top-6 -left-6 text-6xl animate-bounce"
                style={{ animationDelay: "0s" }}
              >
                💌
              </div>
              <div
                className="absolute -bottom-6 -right-6 text-6xl animate-bounce"
              ...
              >
                <span className="relative z-10 flex items-center gap-3">
                  <Heart
                    className={`w-8 h-8 ${loveCount > 0 ? "animate-ping" : ""}`}
                    fill="currentColor"
                  />
                  ส่งรัก ({loveCount})
                </span>
